...
func TestShapingMyanmarSelection(t *testing.T) {
	...
	if err := pdf.Text("ကြိုဆိုပါတယ် ရော ရောင်း သစ္စာ မင်္ဂလာပါ တနင်္ဂနွေ အင်္ဂါ သင်္ကြန် ဒီနေ့တော့ ရေမချိုးတော့ဘူး"); err != nil {
		t.Fatal(err)
	}
	pdf.SetXY(50, 130)

	if err := pdf.Text("ဒီနေ့ နေ့ နွေ နှောင်း နိုး နိုင် နှိုင်း နွှေး လှိုင်း လျှိုး နှိုက် နှီး ဓါတု ဓာတ် နူး နှူး လှိုင်း မှိုင်း နှင့် လှပ် မှား"); err != nil {
		t.Fatal(err)
	}
